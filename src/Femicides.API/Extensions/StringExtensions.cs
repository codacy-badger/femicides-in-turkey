...
namespace Femicides.API.Extensions
{
    public static class StringExtensions
    {
        ...
        public static string PaginateWithParams(this String url, int totalPage, int selectedPage, string queries, int jump)
        {
            if (totalPage > 1 && (totalPage > selectedPage && jump == 1) || (selectedPage > 1 && jump == -1))
            {
                if(string.IsNullOrEmpty(queries))
                {
                    url += "?page=" + (selectedPage + jump).ToString();
                }
                else
                {
                    url += queries + "&page=" + (selectedPage + jump).ToString();
                }
                return url;
            }
            return null;
